...
public class SettingsActivity extends PreferenceActivity {
	...
	public boolean onPreferenceTreeClick(PreferenceScreen preferenceScreen, Preference preference){
		...
		if (!TextUtils.isEmpty(key)){
			...
			if (key.equals(Constants.Preferences.TEST)){
				ReceiverActivity.startReceiverActivity(this, com.infmme.readilyapp.readable.Readable.TYPE_RAW,
	...
	private boolean changeTextViewSize(TextView text, String nSize){
		return StringUtil.isNumeric(nSize) && changeTextViewSize(text, Integer.parseInt(nSize));
	}
					   ...
					   setPositiveButton(android.R.string.ok,
										 new DialogInterface.OnClickListener() {
											 ...
											 public void onClick(DialogInterface dialog, int which){
												 String nSize = String.valueOf(editText.getText()).replaceAll("\\s+", "");
												 if (changeTextViewSize(sampleText, nSize))
													 ...
													 Toast.makeText(context, R.string.illegal_value, Toast.LENGTH_SHORT)
														  .show();
											 }
										 }
					   ).
	...
	private void showFontSizeNumberPicker(final Context context){
		int currentFontSize = Integer.parseInt(PreferenceManager.getDefaultSharedPreferences(context).
				getString(Constants.Preferences.FONT_SIZE, Constants.DEFAULT_FONT_SIZE));
	...
	private void showSpeedEditText(final Context context, final int min, final int max){
		final LinearLayout linearLayout = getDialogLayout(context);
		final EditText editText = new EditText(context);
	...
	@TargetApi(Build.VERSION_CODES.HONEYCOMB)
	@SuppressLint("NewApi")
	private void showSpeedNumberPicker(final Context context, final int min, final int max){
		final NumberPicker numberPicker = new NumberPicker(context);
