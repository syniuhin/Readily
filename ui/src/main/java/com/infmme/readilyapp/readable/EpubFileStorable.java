...
public class EpubFileStorable extends FileStorable {
  ...
  public void process(Context context) {
    try {
      ...
      mBook = (new EpubReader()).readEpubLazy(path, encoding);
      resources = mBook.getContents();
  ...
  public void readData() {
    ...
    while (cycleCount < 5) {
      setText("");
      try {
        while (text.length() < BUFFER_SIZE && resources != null &&
            ! resources.isEmpty() && index < resources.size()) {
  ...
  private String parseEpub(String text) {
    try {
      ...
      if (TextUtils.isEmpty(header)) { header = doc.select("mBook-title").text(); }
