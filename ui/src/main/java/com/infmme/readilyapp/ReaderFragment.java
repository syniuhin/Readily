...
public class ReaderFragment extends Fragment {
  ...
  private static final int NOTIF_SHOWING_LENGTH = 1500; //time in ms for which notification
  // becomes visible
  private static final int READER_PULSE_DURATION = 400; //ms duration of 'Bounce' animation on
  // reader window
  ...
  private void setReaderFontSize() {
    ...
    pointerTop.setPadding((int) (fontSizePx * POINTER_LEFT_PADDING_COEFFICIENT + .5f),
  ...
  private void startReader(final TextParser parser) {
    ...
    if (resultCode == TextParser.RESULT_CODE_OK) {
      final int initialPosition = Math.max(readable.getPosition() - Constants.READER_START_OFFSET,
                                           0);
  ...
  private class Reader implements Runnable {
    ...
    public void run() {
      ...
      if ((position < wordListSize && ! readerTask.isChunkAvailable()) ||
          (position < wordListSize - FileStorable.LAST_WORD_PREFIX_SIZE && readerTask
              .isChunkAvailable())) {
