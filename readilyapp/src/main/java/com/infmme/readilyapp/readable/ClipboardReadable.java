...
public class ClipboardReadable extends Readable {
  ...
  public android.content.ClipboardManager getClipboardNew() { return
      clipboardNew; }

  public void process(final Context context) {
    Looper.prepare(); //TODO: review it CAREFULLY
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
      clipboardNew = (android.content.ClipboardManager) context
          .getSystemService(
          Context.CLIPBOARD_SERVICE);
      processFailed = !clipboardNew.hasText();
      processed = true;
    } else {
      clipboardOld = (ClipboardManager) context.getSystemService(
          Context.CLIPBOARD_SERVICE);
